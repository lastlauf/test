/**
 * Seeds a demo Turkey Slice Invitational: one course, ten players, three
 * finished years of history and a live 2026 tournament mid-round.
 *
 * Run with: npm run seed  (add --force to wipe an existing database)
 */

import { randomBytes, scryptSync } from "node:crypto";
import { db, uid } from "../src/lib/db.ts";

const DEMO_PASSWORD = "TurkeySlice2026!";
...
const already = conn.prepare("SELECT COUNT(*) AS n FROM players").get() as { n: number };
if (already.n > 0) {
  console.log("Database already has players — run `npm run seed -- --force` to reseed.");
  process.exit(0);
}
...
conn
  ...
  .run(courseId, "Turkey Hollow Country Club", "Doylestown", "PA");

const holeInsert = conn.prepare(
  "INSERT INTO holes (id, course_id, number, par, stroke_index, yardage) VALUES (?, ?, ?, ?, ?, ?)",
);
for (const [number, par, si, yards] of HOLES) {
  holeInsert.run(uid("hol"), courseId, number, par, si, yards);
}

const teeInsert = conn.prepare(
  "INSERT INTO tees (id, course_id, name, rating, slope, yardage) VALUES (?, ?, ?, ?, ?, ?)",
);
const blueTee = uid("tee");
teeInsert.run(blueTee, courseId, "Blue", 72.4, 136, 6802);
teeInsert.run(uid("tee"), courseId, "White", 70.8, 131, 6350);
teeInsert.run(uid("tee"), courseId, "Gold", 68.9, 125, 5910);

const PAR = HOLES.reduce((sum, h) => sum + h[1], 0);

// --------------------------------------------------------------- players ---
const playerIds: Record<string, string> = {};
const playerInsert = conn.prepare(
  `INSERT INTO players (id, username, display_name, password_hash, ghin, handicap_index, member_since, is_admin, bio)
   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
);
...
const insertTournament = conn.prepare(
  `INSERT INTO tournaments (id, year, name, course_id, start_date, end_date, status, champion, notes)
   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
);
const insertTeam = conn.prepare(
  "INSERT INTO teams (id, tournament_id, name, color) VALUES (?, ?, ?, ?)",
);
const insertEntry = conn.prepare(
  "INSERT INTO entries (tournament_id, player_id, team_id, course_handicap) VALUES (?, ?, ?, ?)",
);
const insertRound = conn.prepare(
  `INSERT INTO rounds (id, tournament_id, name, format, course_id, tee_id, played_on, sequence, allowance, status)
   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
);
const insertMatch = conn.prepare(
  "INSERT INTO matches (id, round_id, name, sequence, status) VALUES (?, ?, ?, ?, ?)",
);
const insertSide = conn.prepare(
  "INSERT INTO match_sides (id, match_id, label, team_id) VALUES (?, ?, ?, ?)",
);
const insertSidePlayer = conn.prepare(
  "INSERT INTO side_players (side_id, player_id) VALUES (?, ?)",
);
const insertScore = conn.prepare(
  `INSERT INTO scores (id, match_id, hole, subject_type, subject_id, gross, putts, updated_by)
   VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
);
...
const seedAll = conn.transaction(() => {
  for (const plan of PLAN) {
    ...
    insertTeam.run(darkId, tournamentId, "Dark Meat", "#7c2d12");
    insertTeam.run(whiteId, tournamentId, "White Meat", "#0f766e");
    ...
    for (const u of plan.entrants) {
      insertEntry.run(tournamentId, playerIds[u], teamOf[u], null);
    }
    ...
    ROUND_PLAN.forEach((rp, roundIndex) => {
      ...
      pairings.forEach((pairing, matchIndex) => {
        ...
        insertSide.run(darkSide, matchId, "A", darkId);
        insertSide.run(whiteSide, matchId, "B", whiteId);
        for (const u of pairing.dark) insertSidePlayer.run(darkSide, playerIds[u]);
        for (const u of pairing.white) insertSidePlayer.run(whiteSide, playerIds[u]);
...
// ------------------------------------------------------------ side bets ---
if (liveRoundId) {
  const insertWager = conn.prepare(
    "INSERT INTO wagers (id, round_id, match_id, type, amount, settings) VALUES (?, ?, ?, ?, ?, ?)",
  );
  const insertWagerPlayer = conn.prepare(
    "INSERT INTO wager_players (wager_id, player_id) VALUES (?, ?)",
  );
  const firstMatch = conn
    .prepare("SELECT id FROM matches WHERE round_id = ? ORDER BY sequence LIMIT 1")
    .get(liveRoundId) as { id: string };

  const nassauId = uid("wgr");
  insertWager.run(nassauId, liveRoundId, firstMatch.id, "nassau", 20, "{}");
  ...
  for (const u of CURRENT) insertWagerPlayer.run(skinsId, playerIds[u]);

  const h2hId = uid("wgr");
  insertWager.run(h2hId, fourballRoundId, null, "h2h", 50, "{}");
  insertWagerPlayer.run(h2hId, playerIds["dmarchetti"]);
  insertWagerPlayer.run(h2hId, playerIds["rwhitlock"]);
}

const counts = {
  players: (conn.prepare("SELECT COUNT(*) AS n FROM players").get() as { n: number }).n,
  tournaments: (conn.prepare("SELECT COUNT(*) AS n FROM tournaments").get() as { n: number }).n,
  matches: (conn.prepare("SELECT COUNT(*) AS n FROM matches").get() as { n: number }).n,
  scores: (conn.prepare("SELECT COUNT(*) AS n FROM scores").get() as { n: number }).n,
};
console.log("Seeded Turkey Slice Invitational:", counts);
console.log(`Sign in as any username above with password: ${DEMO_PASSWORD}`);
